        ...
        .then(data => {
            ...
            Object.keys(exam).forEach(key => {
                if (key.startsWith('exam')) {
                    ...
                    const questionNumberMatch = key.match(/\d+/);
                    if (!questionNumberMatch) return;

                    const questionNumber = questionNumberMatch[0];
                    const subtopicObj = exam[`subtopic${questionNumber}`];

                    if (!subtopicObj) return;

                    const subtopic = subtopicObj.question_statement;
        ...
        `;
            ...
            createTreeButton.addEventListener('click', () => {
                ...
                fetch('http://127.0.0.1:8000/generate_learning_tree', {
                    ...
                    .then(response => response.json())
                    .then(data => {
                        let treeData = d3.hierarchy(data.tree);
